"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.Entity = void 0;
var tslib_1 = require("tslib");
var context_1 = require("../context");
var getEntityRef_1 = require("./getEntityRef");
//[ ] YONI - in Typescript 5 I do get the name of the class, what would we like to change because of that?
/**Decorates classes that should be used as entities.
 * Receives a key and an array of EntityOptions.
 * @example
 * import  { Entity, Fields } from "remult";
 * @Entity("tasks", {
 *    allowApiCrud: true
 * })
 * export class Task {
 *    @Fields.uuid()
 *    id!: string;
 *    @Fields.string()
 *    title = '';
 *    @Fields.boolean()
 *    completed = false;
 * }
 * @note
 * EntityOptions can be set in two ways:
 * @example
 * // as an object
 * @Entity("tasks",{ allowApiCrud:true })
 * @example
 * // as an arrow function that receives `remult` as a parameter
 * @Entity("tasks", (options,remult) => options.allowApiCrud = true)
 */
function Entity(key) {
    var options = [];
    for (var _i = 1; _i < arguments.length; _i++) {
        options[_i - 1] = arguments[_i];
    }
    return function (target, info) {
        var _a;
        for (var rawFilterMember in target) {
            if (Object.prototype.hasOwnProperty.call(target, rawFilterMember)) {
                var element = target[rawFilterMember];
                if ((_a = element === null || element === void 0 ? void 0 : element.rawFilterInfo) === null || _a === void 0 ? void 0 : _a.rawFilterTranslator) {
                    if (!element.rawFilterInfo.key)
                        element.rawFilterInfo.key = rawFilterMember;
                }
            }
        }
        var factory = function (remult) {
            var e_1, _a;
            var r = {};
            try {
                for (var options_1 = tslib_1.__values(options), options_1_1 = options_1.next(); !options_1_1.done; options_1_1 = options_1.next()) {
                    var o = options_1_1.value;
                    if (o) {
                        if (typeof o === 'function')
                            o(r, remult);
                        else
                            Object.assign(r, o);
                    }
                }
            }
            catch (e_1_1) { e_1 = { error: e_1_1 }; }
            finally {
                try {
                    if (options_1_1 && !options_1_1.done && (_a = options_1.return)) _a.call(options_1);
                }
                finally { if (e_1) throw e_1.error; }
            }
            var base = Object.getPrototypeOf(target);
            if (base) {
                var baseFactory = (0, getEntityRef_1.getEntitySettings)(base, false);
                if (baseFactory) {
                    var opt = baseFactory(remult);
                    if (opt) {
                        r = tslib_1.__assign(tslib_1.__assign({}, opt), r);
                    }
                }
            }
            return r;
        };
        context_1.allEntities.push(target);
        (0, context_1.setControllerSettings)(target, { key: key });
        Reflect.defineMetadata(getEntityRef_1.entityInfo, factory, target);
        Reflect.defineMetadata(getEntityRef_1.entityInfo_key, key, target);
        return target;
    };
}
exports.Entity = Entity;
